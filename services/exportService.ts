...
//  * @returns A promise that resolves with the library object.
//  */
// function waitForGlobal<T>(name: string, timeout = 5000): Promise<T> { // Increased timeout to 5000ms
//     return new Promise((resolve, reject) => {
//         const start = Date.now();
//         const check = () => {
//             const lib = (window as any)[name];
...
//                 resolve(lib as T);
//             } else if (Date.now() - start > timeout) {
//                 reject(new Error(`${name} library not loaded after ${timeout}ms. Please check your network connection or ensure the library script is correctly included in the HTML.`));
//             } else {
//                 setTimeout(check, 100);
//             }
//         };
...
//     } catch (error) {
//         console.error("Failed to export to Excel:", error);
//         alert("Excel export failed. The XLSX library could not be loaded. Please ensure a stable network connection and that the library script is not blocked. Check the browser console for more details.");
//     }
// };
 ...
 * @returns A promise that resolves with the library object.
 */
function waitForGlobal<T>(name: string, timeout = 5000): Promise<T> {
    return new Promise((resolve, reject) => {
        const start = Date.now();
        const check = () => {
            const lib = (window as any)[name];
            if (lib) {
                ...
                reject(new Error(`${name} library not loaded after ${timeout}ms. Please verify that the file '/assets/xlsx.full.min.js' is correctly placed in the public/assets directory and is accessible.`));
            } else {
                setTimeout(check, 100);
            }
        };
...
export const exportToExcel = async (roster: Roster, employees: Employee[], startDate: string, endDate: string, fileName: string) => {
    try {
        ...
        XLSX.writeFile(workbook, fileName, { type: 'binary', bookType: 'xlsx' });

    } catch (error) {
        console.error("Failed to export to Excel:", error);
        alert("Excel export failed. The SheetJS library could not be loaded. Please ensure the file '/assets/xlsx.full.min.js' is correctly placed in the public/assets directory and check the browser console for more details.");
    }
};